...
from .diff_module.NSDE import LatentSDE, NeuralSDE, NeuralTSDE, Diffusion_model, FinalTanhTX
...
## list of tsde models
tsde_models = [x for y in [[['{}_{}_{}'.format(s,i,j) for s in sde_models] for i in range(4)] for j in range(7)] for x in y]
tsde_models = [x.replace('neuralsde', 'neuraltsde') for y in tsde_models for x in y]
model_name_list = model_name_list + tsde_models
...
class ists_layer(nn.Module):
    def __init__(self, model_name='cnn', input_dim=3, seq_len=128, 
        ...
        elif self.model_name in tsde_models:
            input_option = int(self.model_name.split('_')[1])
            noise_option = int(self.model_name.split('_')[2])
            initial_option = int(self.model_name.split('_')[3])
            attention_option = int(self.model_name.split('_')[4])
            
            if (input_option == 0) & (noise_option == 0):
                vector_field = FinalTanhTX(input_channels=self.coeff_dim, hidden_channels=self.hidden_dim,
                                           hidden_hidden_channels=self.hidden_dim, num_hidden_layers=self.num_hidden_layers)  
            else:
                vector_field = Diffusion_model(input_channels=self.coeff_dim, hidden_channels=self.hidden_dim,
                                               hidden_hidden_channels=self.hidden_dim, num_hidden_layers=self.num_hidden_layers, 
                                               input_option=input_option, noise_option=noise_option)
            self.seq_layer = NeuralTSDE(func=vector_field, input_channels=self.coeff_dim,
                                          hidden_channels=self.hidden_dim, output_channels=self.hidden_dim, initial=True, initial_option=initial_option, attention_option=attention_option)
    ...
    def forward(self, seq, coeffs):
        ...
        if self.model_name not in model_name_list:
            ...
            out, hn = self.seq_layer(x, seq, coeffs, times, **kwargs)
