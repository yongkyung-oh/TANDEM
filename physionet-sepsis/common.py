...
def _evaluate_metrics(dataloader, model, times, loss_fn, num_classes, device, kwargs):
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            seq, coeffs, static, true_y, lengths = batch
            batch_size = true_y.size(0)
            pred_y = model(seq, times, coeffs, static, lengths, **kwargs)
    ...
    for epoch in tqdm_range:
        ...
        for batch in train_dataloader:
            ...
            with _SuppressAssertions(tqdm_range):
                train_seq, train_coeffs, train_static, train_y, lengths = batch
                pred_y = model(train_seq, times, train_coeffs, train_static, lengths, **kwargs)
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            ...
            try:
                tqdm_range.write('Epoch: {}  Train loss: {:.3}  Train accuracy: {:.3}  Train auroc: {:.3}  '
                                 ...
                                 ''.format(epoch, train_metrics.loss, train_metrics.accuracy, train_metrics.auroc, 
                                           val_metrics.loss, val_metrics.accuracy, val_metrics.auroc))
            except:
...
def main(args, name, model_name, times, train_dataloader, val_dataloader, test_dataloader, device, make_model, num_classes, max_epochs,
         lr, kwargs, step_mode, pos_weight, reg, out_name):
...
def make_model(name, input_channels, output_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers,
    ...
    sde_models = [
        'neuralsde_0_00', # neural cde
        # 'neuralsde_1_00', # naive ode 
        'neuralsde_3_00', # naive ode + t 
        # 'neuralsde_1_18', # naive sde
        'neuralsde_3_18', # naive sde + t
        'neuralsde_2_16', # lsde (additive noise)
        # 'neuralsde_4_16', # lsde+t (additive noise)
        'neuralsde_4_17', # lnsde (multiplicative noise)
        'neuralsde_6_17', # gsde (multiplicative noise)
    ]

    tsde_models = [x for y in [[['{}_{}_{}'.format(s,i,j) for s in sde_models] for i in range(4)] for j in range(12)] for x in y]
    tsde_models = [x.replace('neuralsde', 'neuraltsde') for y in tsde_models for x in y]
    
    ## 
    if name in sde_models:
        input_option = int(name.split('_')[1])
        noise_option = int(name.split('_')[2])
    ...
    ##
    elif name in tsde_models:
        input_option = int(name.split('_')[1])
        noise_option = int(name.split('_')[2])
        initial_option = int(name.split('_')[3])
        attention_option = int(name.split('_')[4])

        def make_model():
            if (input_option == 0) & (noise_option == 0):
                vector_field = models.FinalTanhTX(input_channels=input_channels, hidden_channels=hidden_channels,
                                                  hidden_hidden_channels=hidden_channels, num_hidden_layers=num_hidden_layers)  
            else:
                vector_field = models.Diffusion_model(input_channels=input_channels, hidden_channels=hidden_channels,
                                                      hidden_hidden_channels=hidden_channels, num_hidden_layers=num_hidden_layers, 
                                                      input_option=input_option, noise_option=noise_option)
            model = models.NeuralTSDE(func=vector_field, input_channels=input_channels,
                                      hidden_channels=hidden_channels, output_channels=output_channels, initial=True, initial_option=initial_option, attention_option=attention_option)
            return model, vector_field
